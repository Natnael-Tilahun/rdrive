...
import Star from '../Comments/Star';

const FolderCard = ({ date }) => {
  const { query, asPath } = useRouter();
  let clean = asPath;
  if (asPath.includes('#')) {
    clean = asPath.split('#')[0];
  }
  ...
  const URL = `/api/thumbnail/?path=${clean}icon.png`;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col w-full h-full px-2 mt-2 sm:mt-0 sm:p-1 md:pr-5">
        ...
        <p className="text-[15px] my-1">{description}</p>
        <div className="flex items-center justify-between my-2 px-0.5 space-x-0.1">
          <p className="text-[16px] text-gray-500 dark:text-gray-500">{date}</p>
